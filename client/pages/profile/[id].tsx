...
const Profile: NextPage = () => {
  ...
  //カバー画像
  const [coverPicture, setCoverPicture] =
    useState<File | null>(null);
  ...
  return (
    <Layout title='Profile'>
      <div className='lg:w-256 relative h-60 w-full'>
        <Menu
          shadow='md'
      ...
      >
        ...
        {loginUser?.id === user.id ? (
          <Menu.Dropdown>
            <form
              onSubmit={(e) =>
                handleSubmitUserName(e, user, userNameInput)
              }
            >
      ...
      <div className='mt-10 flex  w-full items-center justify-center'>
        {loginUser?.id === user.id ? (
          display === 'question' ? (
            ...
            <ChartContainer isMine={true} userId={user.id} />
          ) : (
            <></>
          )
        ) : display === 'question' ? (
          ...
          <ChartContainer isMine={false} userId={user.id} />
        ) : (
          <></>
        )}
      </div>
